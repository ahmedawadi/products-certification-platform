  ...
  const copyToClipboard = async () => {
    try {
      ...
      toast.success("Hash copié", {
        description:
          "Le hash de transaction a été copié dans le presse-papiers.",
      });
    } catch (err) {
      toast.error("Erreur", {
    ...
    >
      ...
      <div className="text-center">
        ...
        <h3 className="text-xl font-semibold text-foreground mb-2">
          Produit certifié avec succès !
        </h3>
        <p className="text-muted-foreground text-pretty">
          <span className="font-medium">{productName}</span> a été ajouté à la
          blockchain et certifié.
        </p>
      </div>
